...
public class OModel implements ISyncServiceListener {
    ...
    public String BASE_AUTHORITY = "com.odoo.experience.core.provider.content";
    ...
    public OModel(Context context, String model_name) {
        mContext = context;
        mUser = OdooUser.current(context);
        this.model_name = model_name;
        if (mUser != null) {
            mOdooVersion = mUser.getOdooVersion();
            if (sqLite == null) {
                sqLite = new OSQLite(mContext);
            }
        }
    }
    ...
    public static OModel createInstance(Context context, Class<?> type) {
        try {
            Constructor<?> constructor = type.getConstructor(Context.class);
            return (OModel) constructor.newInstance(context);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private String[] updateProjection(String[] projection) {
        ...
        if (allProjection == null) {
            ...
            for (String col : projection) {
                ...
                if (column != null && column.isFunctionalColumn() && column.canFunctionalStore()) {
    ...
    private List<OColumn> getFunctionalColumns(String[] projection) {
        ...
        if (projection != null) {
            ...
            for (OColumn column : getFunctionalColumns()) {
                if (column != null && !column.canFunctionalStore())
                    cols.add(column);
            }
        }
        return cols;
    ...
    public Cursor executeRawQuery(String sql, String[] args) {
        SQLiteDatabase db = getReadableDatabase();
