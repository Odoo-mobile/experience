...
import com.odoo.core.account.AboutApp;
...
public class DrawerUtils {
    ...
    public static List<ODrawerItem> baseSettingsItems(Context context) {
        ...
        settings.add(new ODrawerItem(key).setTitle(OResource.string(context, R.string.title_about))
                .setIcon(R.drawable.ic_icon_info)
                .setInstance(AboutApp.class));
        return settings;
    }

    public static View fillDrawerItemValue(View view, ODrawerItem item) {
        if (item.isGroupTitle()) {
            OControls.setGone(view, R.id.group_title);
        } else {
            if (item.getIcon() > 0)
